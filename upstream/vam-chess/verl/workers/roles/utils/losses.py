...
def ppo_loss(config: ActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    response_mask = data["response_mask"].to(bool)
    forced_mask = data.get("forced_token_mask", None)
    if forced_mask is not None:
        forced_mask = forced_mask.to(log_prob.device).to(bool)
    # compute policy loss
    ...
    if forced_mask is not None:
        old_log_prob = torch.where(forced_mask, log_prob.detach(), old_log_prob)
    ...
    if config.use_kl_loss:
        ...
        if forced_mask is not None:
            ref_log_prob = torch.where(forced_mask, log_prob.detach(), ref_log_prob)
        # compute kl loss
