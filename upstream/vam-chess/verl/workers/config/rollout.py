...
class RolloutConfig(BaseConfig):
    ...
    top_k: int = -1
    top_p: float = 1.0
    # Optional list of stop strings (vLLM/SGLang SamplingParams.stop).
    # Useful for enforcing strict output contracts (e.g., stop after a closing tag).
    stop: Optional[list[str]] = None
    # If using stop strings, include the stop string itself in the output.
    # This is important for strict tag-based contracts where the stop marker is a required closing tag.
    ...
    # Optional response-side prefix. When enabled, rollout backends append this
    # token prefix to the generation prompt and also prepend the same token IDs
    # to the returned response tensor, so reward/logging decode sees the prefix
    # as response text rather than prompt text.
    response_prefix_enable: bool = False
