    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if "multi_modal_data" in non_tensor_batch:
            ...
            for raw_prompt_ids, multi_modal_data in zip(
                ...
                sglang_inputs.append(
                    {
                        "prompt_token_ids": list(raw_prompt_ids) + forced_prefix_tokens[len(sglang_inputs)],
        ...
        if any(forced_prefix_tokens):
            ...
            for i in range(batch_size):
                ...
                resp_ids = response[i].tolist() if i < response.size(0) else []
                combined = (list(prefix_ids) + list(resp_ids))[:max_resp_len]
                full_response_rows.append(torch.tensor(combined, dtype=response.dtype, device=idx.device))
                ...
                forced_mask_rows.append(forced_row)

                if self.config.calculate_log_probs:
                    lp_tail = rollout_log_probs[i].tolist() if i < rollout_log_probs.size(0) else []
                    combined_lp = ([0.0] * forced_len + lp_tail)[:max_resp_len]
                    full_logprob_rows.append(torch.tensor(combined_lp, dtype=torch.float32, device=idx.device))

            response = pad_sequence(full_response_rows, batch_first=True, padding_value=self.pad_token_id)
            if response.shape[1] < max_resp_len:
                response = pad_sequence_to_length(response, max_resp_len, self.pad_token_id)
            elif response.shape[1] > max_resp_len:
                response = response[:, :max_resp_len]
            forced_token_mask = torch.tensor(forced_mask_rows, device=idx.device, dtype=torch.bool)

            if self.config.calculate_log_probs:
                rollout_log_probs = pad_sequence(
                    full_logprob_rows, batch_first=True, padding_value=self.pad_token_id
                ).to(idx.device)
                if rollout_log_probs.shape[1] < max_resp_len:
                    rollout_log_probs = pad_sequence_to_length(rollout_log_probs, max_resp_len, self.pad_token_id)
                elif rollout_log_probs.shape[1] > max_resp_len:
                    rollout_log_probs = rollout_log_probs[:, :max_resp_len]
        ...
        if forced_token_mask is None:
            forced_token_mask = torch.zeros(
                (batch_size, self.config.response_length), device=idx.device, dtype=torch.bool
            )
