...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True, n_micro_batches=1):
        ...
        if sample_weight is not None:
            # Apply per-example weights by scaling the token loss mask.
            # This keeps the trainer's "token-mean" normalization semantics, but weighted.
            sample_weight = sample_weight.to(self.device_name).to(torch.float32)
            if sample_weight.dim() == 0:
                sample_weight = sample_weight.view(1)
            batch_size, seqlen = input_ids.shape
            # Align weights with shifted tokens (we drop the first token in loss_mask via [:, 1:]).
            token_weight = sample_weight.view(batch_size, 1).expand(batch_size, max(seqlen - 1, 0)).reshape(-1)
            if token_weight.numel() == loss_mask.numel():
                loss_mask = loss_mask.to(token_weight.device) * token_weight
    ...
    def fit(self):
        ...
        # Optional explicit checkpoint step list.
        save_steps_set = None
        save_steps = getattr(self.config.trainer, "save_steps", None)
        if save_steps is not None:
            try:
                save_steps_set = {int(x) for x in save_steps}
            except Exception:
                raise ValueError(f"trainer.save_steps must be a list of ints, got: {save_steps!r}")
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            for step_in_epoch, data in enumerate(
                ...
                # - Legacy: save every `save_freq` steps when save_freq > 0.
                # - Optional: save at explicit `trainer.save_steps` (list[int]) when provided.
                if save_steps_set is not None:
                    is_save_step = global_step in save_steps_set
                else:
                    save_freq = int(getattr(self.config.trainer, "save_freq", -1))
