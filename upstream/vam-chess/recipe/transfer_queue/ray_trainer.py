...
def _compute_reward_diag_means(reward_extra_infos_dict: dict[str, Any] | None) -> dict[str, float]:
    """Return mean diagnostics for chess reward logging."""
    ...
    for friendly_name, keys in metric_candidates.items():
        for key in keys:
            ...
            if arr.size == 0:
                continue
            stats[friendly_name] = float(arr.mean())
            break
    return stats
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        global_reward_means = _compute_reward_diag_means(reward_extra_infos_dict)
        for key, value in global_reward_means.items():
            metric_dict[f"val-aux/all/{key}/mean"] = value
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                metrics = {}
                reward_logging_means: dict[str, float] = {}
                reward_tensor = None
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        reward_logging_means = _compute_reward_diag_means(reward_extra_infos_dict)
                ...
                if reward_logging_means:
                    reward_metric_name_map = {
                        "total_reward": "train/reward/total_mean",
                        "format_reward": "train/reward/format_mean",
                        "hint_penalty": "train/reward/hint_penalty_mean",
                    }
                    for key, metric_name in reward_metric_name_map.items():
                        value = reward_logging_means.get(key)
