...
class ChessEnvironmentManager(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, config):
        self.memory = SimpleMemory()
        super().__init__(envs, projection_f, config)
        ...
        self.memory.store(
            {
                ...
                "action": [a if a else "invalid" for a in actions],
            }
        )
        self.pre_text_obs = next_obs
        next_observations = {
    ...
    def build_text_obs(self, infos, text_obs: List[str] = None, init: bool = False) -> List[str]:
        ...
        for i in range(len(infos)):
            if init or self.config.env.history_length <= 0:
                obs = CHESS_TEMPLATE_NO_HIS.format(current_observation=text_obs[i])
            else:
                obs = CHESS_TEMPLATE.format(
                    step_count=len(self.memory[i]),
                    history_length=valid_lens[i],
                    action_history=memory_contexts[i],
                    current_step=len(self.memory[i]) + 1,
...
class ChessLessonEnvironmentManager(EnvironmentManagerBase):
    """Single-turn manager for the lichess "Learn chess" puzzles.

    Each task is self-describing (board, goal, targets are in the worker obs), so
    there is no cross-step history to maintain; the episode is exactly one step.
    """
    ...
    def reset(self, kwargs):
        obs, infos = self.envs.reset()
        observations = {
            "text": [CHESSLESSON_TEMPLATE_NO_HIS.format(current_observation=o) for o in obs],
            "image": None,
            "anchor": obs,
        }
        return observations, infos
    ...
    def step(self, text_actions: List[str]):
        ...
        for i, info in enumerate(infos):
            info["is_action_valid"] = to_numpy(valids[i])
        next_observations = {
            "text": [CHESSLESSON_TEMPLATE_NO_HIS.format(current_observation=o) for o in next_obs],
            "image": None,
            "anchor": next_obs,
        }
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        return envs, val_envs
    elif "chesslesson" in config.env.env_name.lower():
        from chess_game import build_chesslesson_envs, chesslesson_projection
        ...
        _val_envs = build_chesslesson_envs(
        ...
        projection_f = partial(chesslesson_projection)
        envs = ChessLessonEnvironmentManager(_envs, projection_f, config)
        val_envs = ChessLessonEnvironmentManager(_val_envs, projection_f, config)
        return envs, val_envs
    elif "chess" in config.env.env_name.lower():
        from chess_game import build_chess_envs, chess_projection

        env_kwargs = {"max_agent_plies": config.env.chess.max_agent_plies}
        ...
        projection_f = partial(chess_projection)
        envs = ChessEnvironmentManager(_envs, projection_f, config)
        val_envs = ChessEnvironmentManager(_val_envs, projection_f, config)
        return envs, val_envs
        ...
        parquets = OmegaConf.to_container(lp_cfg.parquets, resolve=True)
        env_kwargs = {"parquets": list(parquets)}
        vam_cfg = getattr(lp_cfg, "vam", None)
        if vam_cfg is not None:
            try:
                env_kwargs["vam"] = OmegaConf.to_container(vam_cfg, resolve=True)
            except Exception:
        ...
        _envs = build_lichess_puzzle_envs(
            config.env.seed,
            config.data.train_batch_size,
            group_n,
            resources_per_worker=resources_per_worker,
        ...
        _val_envs = build_lichess_puzzle_envs(
            config.env.seed + 1000,
            config.data.val_batch_size,
            1,
            resources_per_worker=resources_per_worker,
        ...
        projection_f = partial(lichess_puzzle_projection)
        envs = ChessEnvironmentManager(_envs, projection_f, config)
        val_envs = ChessEnvironmentManager(_val_envs, projection_f, config)
        return envs, val_envs
