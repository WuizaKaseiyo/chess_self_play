    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if len(var_vals) == 0:
                    continue
                # Special handling for string-based diagnostics
                if isinstance(var_vals[0], str):
                    # Provide useful summaries for string diagnostics from reward functions
                    n_total = len(var_vals)
                    if var_name == "reason":
                        # reason categories emitted by the chess reward_fn: {exact_match, legal_not_gt, illegal, parse_fail}
                        cats = [
                            "exact_match",
                            "legal_not_gt",
                            "illegal",
                        ...
                        metric = {}
                        for cat in cats:
                            cnt = sum(1 for v in var_vals if v == cat)
                            metric[f"{cat}_ratio@{n_total}"] = (cnt / n_total) if n_total > 0 else 0.0
                            metric[f"{cat}_count@{n_total}"] = float(cnt)
                        data_src2uid2var2metric[data_source][uid][var_name] = metric
                        continue
                    if var_name == "parsed_as":
                        # parsed_as is one of {uci, san, unknown}
                        cats = ["uci", "san", "unknown"]
                        metric = {}
                        for cat in cats:
                            cnt = sum(1 for v in var_vals if v == cat)
                            metric[f"{cat}_ratio@{n_total}"] = (cnt / n_total) if n_total > 0 else 0.0
                        ...
                        data_src2uid2var2metric[data_source][uid][var_name] = metric
                        continue
                    # Other string vars are skipped
                    continue

                # Filter to numeric values only; ignore None/NaN and non-numeric entries
                numeric_mask = []
                numeric_vals = []
                for idx, v in enumerate(var_vals):
                    if isinstance(v, (int, float, bool, np.floating)) and not (
                        isinstance(v, float) and math.isnan(v)
                    ):
                        numeric_mask.append(idx)
                        numeric_vals.append(float(v))

                if len(numeric_vals) == 0:
                    # Nothing numeric to aggregate for this variable/uid
                    continue
                ...
                # Record presence/coverage to make missing values explicit in logs
                missing = n_total - n_resps
                metric[f"valid_count@{n_total}"] = float(n_resps)
                metric[f"missing_count@{n_total}"] = float(missing)
                metric[f"valid_ratio@{n_total}"] = (n_resps / n_total) if n_total > 0 else 0.0
                ...
                if n_resps > 1:
                    ...
                    for n in ns:
                        ...
                        if var2vals.get("pred", None) is not None:
                            # Align preds to the numeric subset
                            preds = var2vals["pred"]
                            vote_pairs = [
                                {"val": numeric_vals[j], "pred": preds[i]} for j, i in enumerate(numeric_mask)
                            ]
                            if len(vote_pairs) < n:
                                # Not enough aligned pairs for this subset size; skip majority metric
                                continue
