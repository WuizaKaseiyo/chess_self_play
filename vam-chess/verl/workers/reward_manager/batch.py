...
class BatchRewardManager(AbstractRewardManager):
    ...
    def verify(self, data):
        ...
        for i in range(len(data)):
            ...
            prompt_valid_len = int(valid_prompt_lengths[i].item())
            if prompt_valid_len > 0:
                prompt_valid_ids = prompt_ids[i][-prompt_valid_len:]
                prompt_str = self.tokenizer.decode(prompt_valid_ids, skip_special_tokens=True)
            else:
                prompt_str = ""
            prompts_str.append(prompt_str)

        ground_truths = [item.non_tensor_batch["reward_model"].get("ground_truth", None) for item in data]
        # Payload passed to the custom reward fn (e.g., "reward_model").
        ...
        for i in range(len(data)):
            extra_i = extras[i] if i < len(extras) else {}
            if not isinstance(extra_i, dict):
                extra_i = {}
            extra_i["rollout_reward_scores"] = rollout_reward_scores[i]
            extra_i["prompt_text"] = prompts_str[i]
            extras[i] = extra_i
    ...
    def __call__(self, data: DataProto, return_dict: bool = False) -> torch.Tensor | dict[str, Any]:
        ...
        valid_response_lengths = attention_mask[:, prompt_len:].sum(dim=-1)
        data_sources = data.non_tensor_batch[self.reward_fn_key]
        # For logging, group by the dataset source string, independent from the reward payload type.
        data_source_keys = data.non_tensor_batch.get("data_source", ["<unknown>"] * len(data))
        ...
        for i in range(len(data)):
            ...
            data_source = data_source_keys[i]
