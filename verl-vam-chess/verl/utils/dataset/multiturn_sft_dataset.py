...
class MultiTurnSFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: str | list[str], tokenizer, config=None, max_samples: int = -1):
        ...
        # Optional per-example weight (e.g., for AWR-style weighted SFT datasets).
        # When set, the dataset returns `sample_weight` as a float32 tensor.
        self.weight_key = multiturn_config.get("weight_key", None)
    ...
    def _read_files_and_process(self):
        ...
        if self.weight_key is not None:
            if self.weight_key not in self.dataframe.columns:
                raise KeyError(
                    f"weight_key={self.weight_key!r} not found in parquet columns: {list(self.dataframe.columns)}"
                )
            self.sample_weights = self.dataframe[self.weight_key].apply(series_to_item).tolist()
        else:
