...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_chat_template = as_bool(config.get("use_chat_template", True), default=True)
    ...
    def maybe_filter_out_long_prompts(self, dataframe: datasets.Dataset = None):
        ...
        if self.filter_overlong_prompts:
            ...
            if processor is not None:
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        _, prompt_ids = encode_prompt_from_messages(
                            tokenizer,
                            doc[prompt_key],
            ...
            # NOTE: HuggingFace Datasets uses a multiprocessing Pool when `num_proc` is set,
            # even for `num_proc=1`. On some HPC nodes / container setups this can fail with:
            #   OSError: [Errno 28] No space left on device
            # while creating semaphore-backed queues (SemLock).
            #
            # For `num_proc<=1`, avoid Pool entirely by not passing `num_proc` so the filter
            # runs in-process.
            filter_kwargs = {"desc": f"Filtering prompts longer than {self.max_prompt_length} tokens"}
            if int(self.num_workers or 0) > 1:
                filter_kwargs["num_proc"] = int(self.num_workers)
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            if self.use_chat_template and self.apply_chat_template_kwargs.get("chat_template") is None:
                assert getattr(self.tokenizer, "chat_template", None) is not None, (
