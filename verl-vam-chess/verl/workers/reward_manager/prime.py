...
class PrimeRewardManager(AbstractRewardManager):
    ...
    def verify(self, data):
        ...
        extra_info = data.non_tensor_batch.get("extra_info", [{} for _ in range(len(data))])

        prompt_texts: list[str] = []
        for i in range(len(data)):
            vlen = int(valid_prompt_lengths[i].item())
            if vlen > 0:
                prompt_valid_ids = prompt_ids[i][-vlen:]
                prompt_texts.append(self.tokenizer.decode(prompt_valid_ids, skip_special_tokens=True))
            else:
                prompt_texts.append("")

        for i in range(len(data)):
            extra_i = extra_info[i] if i < len(extra_info) else {}
            if not isinstance(extra_i, dict):
                extra_i = {}
            extra_i["prompt_text"] = prompt_texts[i]
